...
\begin{align}
  \lVert y - X\beta \rVert^2  - \lVert y - X\hat{\beta} \rVert^2
    &= (\hat{\beta}-\beta)^TX^TX(\hat{\beta}-\beta)\\
    &= (\hat{\beta}-\beta)^TP^TL_{\theta}L_{\theta}^TP(\hat{\beta}-\beta)\\
    &= \left[L_{\theta}^TP(\hat{\beta}-\beta)\right]^T\left[L_{\theta}^TP(\hat{\beta}-\beta)\right]\\
    &= \lVert L_{\theta}^TP(\hat{\beta}-\beta)\rVert^2
\end{align}
$$
